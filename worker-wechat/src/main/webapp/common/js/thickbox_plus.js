...
function TB_launch() {
	$("a.thickbox").click(function() {
				...
				if($("#pcmenber").val()==null || $("#pcmenber").val()==''){
			  		alert("请先进行登录操作！");
			  		return;
			  	}
				...
				if($("#amount").val!=''){
					...
					$.ajax({
						 type:"post",
						 dataType:"json",
						 url:$("#urlPre").val()+"/pub/shopcar/updateShopCar.do",
						 data:"goodId="+$("#goodId").val()+"&shopType=add&type=web&amount="+$("#amount").val(),
						 async:false,
						 complete: function (XMLHttpRequest,textStatus) {
						    if(XMLHttpRequest.responseText!="" && XMLHttpRequest.responseText==0){
						    	href=$("#urlPre").val()+"/web/shopCarMessage.html?height=130;width=480";
								TB_show(t, href);
								return false;
						    }
						 }
...
function TB_show(caption, url) { 
	try {
		...
		$("body")
				.append(
						"<div id='TB_load'><div id='TB_loadContent'><img src='images/circle_animation.gif' /></div></div>");
		$("#TB_overlay").show();
		var urlString = /.jpg|.jpeg|.png|.gif|.html|.htm/g;
		var urlType = url.match(urlString);

		if (urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png'
				|| urlType == '.gif') {// code to show images
			...
			imgPreloader.onload = function() {
				...
				if (imgPreloader.width > x) {
					imgPreloader.height = imgPreloader.height
							* (x / imgPreloader.width);
					imgPreloader.width = x;
					if (imgPreloader.height > y) {
						imgPreloader.width = imgPreloader.width
								* (y / imgPreloader.height);
				...
				$("#TB_window")
						.append(
								"<img id='TB_Image' src='"
										...
										+ "<div id='TB_caption'>"
										+ caption
										+ "</div><div id='TB_closeWindow'><a href='#' id='TB_closeWindowButton'>关闭</a></div>");
				$("#TB_closeWindowButton").click(TB_remove);
				$("#TB_Image").click(TB_remove); // close when image clicked
				// added by Christian
				// Montoya
				TB_position();
				$("#TB_load").remove();
		...
		if (urlType == '.htm' || urlType == '.html' || urlType == 'jsp') {// code
			// to
			// show
			// html
			// pages
			...
			$("#TB_window")
					.append(
							"<div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton'>关闭</a></div><div id='TB_ajaxContent' style='width:"
									+ ajaxContentW
									+ "px;height:"
									+ ajaxContentH + "px;'></div>");
			$("#TB_closeWindowButton").click(TB_remove);
...
function TB_remove() {
	// #TB_load removal added by Christian Montoya; solves bug when overlay is
	// closed before image loads
