...
*/
(function ($) {
	$.fn.extend({floatDiv:function (opt) {
		if ($(this).length == 0) {
			return;
		}
		opt = $.extend({onOff:true, width:"", height:"", cover:true, coverDiv:"floatDivCover", coverColor:"#000000", fade:50, space:0, level:"center", vertical:"middle", closeHandler:"", dragHandler:"", dragLimit:true, ifScroll:true, scrollBar:false}, opt);
		var obj = $(this), selfWidth, selfHeight, maxZ = 0, browserVersion = null, dragX = 0, dragY = 0, dragFlag = false;
		var init = function () {
		...
		var coverFix = function () {
			var doc = document.compatMode ? document.documentElement : document.body, scrollWidth = doc.scrollWidth, scrollHeight = doc.scrollHeight;
		...
		var keepfix = function (level, vertical) {
			var winWidth = $(window).width(), winHeight = $(window).height(), scrollLeft = $(window).scrollLeft(), scrollTop = $(window).scrollTop(), _left_l = opt.space, _left_c = (winWidth - selfWidth) / 2, _left_r = winWidth - selfWidth - opt.space, _top_t = opt.space, _top_m = (winHeight - selfHeight) / 2, _top_b = winHeight - selfHeight - opt.space;
			...
			if (browserVersion == "ie6") {
				...
				if (!opt.cover && hideSelect.length) {
					$("#framezd").css({"z-index":maxZ + 1, "left":_left + "px", "top":_top + "px"});
				}
			}
			obj.css({"z-index":maxZ + 3, "left":_left + "px", "top":_top + "px"});
		};
		var dragEvent = function () {
			var dragHandler = opt.dragHandler == "self" ? obj : obj.find(opt.dragHandler), _move = false, posX = 0, posY = 0, x = 0, y = 0;
			...
			$(document).mousemove(function (ev) {
				if (_move) {
					...
					y = ev.pageY - posY;
					dragFlag = true;
					var x_Boundary = $(window).width() - obj.width(), y_Boundary = $(document).height() - obj.height();
					if (opt.dragLimit) {
						x = x < 0 ? 0 : x;
		...
		var closeEvent = function () {
			...
			if (hideSelect && hideSelect.length) {
				hideSelect.show();
			}
		};
