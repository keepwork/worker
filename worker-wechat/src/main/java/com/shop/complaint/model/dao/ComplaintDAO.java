package com.shop.complaint.model.dao;
 ...
 */
public class ComplaintDAO extends DaoSupport
{
	public void save(ComplaintDTO dto)
	{
	...
	{
		String hql = "from ComplaintDTO Z where Z.id=?";
		List list = this.getHibernateTemplate().find(hql, id);
		if (list.size() == 1)
			return (ComplaintDTO) list.get(0);
		else
			return null;
	}

	public void delete(ComplaintDTO dto)
	{
	...
	public void update(ComplaintDTO dto)
	{
		this.getHibernateTemplate().update(dto);
	}

	public void saveOrUpdate(ComplaintDTO dto)
	{
	...
	public List list(LimitInfo limit)
	{
		...
		String hql = "from ComplaintDTO Z ";
		...
		{// 分页查询
			...
			// 查询并设置记录总数
			String totalHQl = "from ComplaintDTO Z";
	...
	{
        final String sql = "select t.ID,t.COMP_TIME,t.STATUS from pub_complaint t where t.MEN_ID = '"+params.get("menId")+"' " +
	...
	{
		final String sql = "select t.ID from pub_complaint t where t.MEN_ID = '"+params.get("menId")+"' " ;
		List list = (List) this.getHibernateTemplate().execute(
		        new HibernateCallback() {
		            @Override
		            public Object doInHibernate(Session arg0)
	...
	{
		return this.getHibernateTemplate().find("from ComplaintDTO Z where Z.id in (" + ids + ")");
	}
