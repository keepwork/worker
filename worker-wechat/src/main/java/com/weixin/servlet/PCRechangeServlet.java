 ...
 * @date 2015-06-28
 */
public class PCRechangeServlet extends HttpServlet {
...
//		System.out.println("price:"+price);
		...
		String pcmenberId = (String)request.getSession().getAttribute("pcmenberId"); 
//		System.out.println("pcmenberId:"+pcmenberId);
		
		String out_trade_no = UUID.getUUID();//充值序列号
		request.getSession().setAttribute("squeues", out_trade_no);
    	String attach = pcmenberId;//附加数据，存放用户的menId
...
//		System.out.println("=====================xml");
		...
		System.out.println("PC会员充值生成二维码....................................................code_url=" + code_url);
		
		//生成二维码图片 
		String rootPath = GlobalConfig.getProperty("weixin", "image.root");
		String codeImageName = UUID.getUUID();
		request.getSession().setAttribute("rechangeImageName", codeImageName);
		try {
			QRCodeUtil.encode(code_url,rootPath + "/common/imagesCodeRechange/"+codeImageName+".jpg");
		} catch (Exception e) {
		...
		System.out.println("PC会员充值生成二维码完成....................................................(" +
					"menId="+pcmenberId+",code_url:"+code_url+","+format.format(new Date())+")");
		
		//正式目录跳转
		response.sendRedirect(serverDomain+"/web/payRecharge.jsp?squeues="+codeImageName);
