...
function addAttachmentField() {
	if (attachmentCounter < attachmentLimit) {
		var s = attachmentTemplate.replace(/#attachmentCounter#/g,
				attachmentTotal);
...
function del_selecetd() {
	...
	if (selectObj.selectedIndex != -1
			&& selectObj.selectedIndex <= selectObj.length) {
...
function extvalidate(filepath) {
	...
	if (tempExt != "html" && tempExt != "htm" && tempExt != "xml"
			&& tempExt != "txt" && tempExt != "doc" && tempExt != "rtf"
			&& tempExt != "pdf" && tempExt != "jpeg" && tempExt != "gif"
			&& tempExt != "zip" && tempExt != "jpg") {
		return false;
	}
	return true;
}
...
function showChannelTreeSingleSelect() {
	...
	var url = "/cms4/cms/channel/treeCmsChannelSingleSelect.do?selectNodeNo="
			+ selectNodeNo;
	var retValue = window.showModalDialog(url, window,
...
function chooseSubject(ctrlobj, webContext, event) {
	...
	// alert(url);
	retval = window.showModalDialog(url, "", "dialogWidth:" + width
			+ "px; dialogHeight:" + height + "px; dialogLeft:" + showx
			+ "px; dialogTop:" + showy
			+ "px; status:yes; directories:yes;scrollbars:yes;Resizable=yes;");
	if (retval != null) {
