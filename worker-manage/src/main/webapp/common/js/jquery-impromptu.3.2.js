...
function conformDel(delUrl)
{
    $.prompt('您确定要删除此信息吗!!',{ buttons:{'确定':true, '取消':false}, submit: function(v,m,f){ 
        if(v){
		    $.ajax({
			      type:"get",
			      dataType:"json",
			      ...
			      success: function(data){
			        $.prompt(data.msg,{ buttons:{'确定':true}, submit: function(v,m,f){ 
				      if(v){
				          if( data.code == '0' ){     //删除成功时候才刷新
...
function conformUpdate(title,url)
{
	$.prompt(title,{ buttons:{'确定':true, '取消':false}, submit: function(v,m,f)
    { 
        if(v)
        {
		    $.ajax({
...
function conformUpdateWap(title,url)
{
	$.prompt(title,{ buttons:{'确定':true, '取消':false}, submit: function(v,m,f)
    { 
        if(v)
