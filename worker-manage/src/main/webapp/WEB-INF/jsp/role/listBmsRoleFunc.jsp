...
<html>
	<head>
		...
		<link
			href="${pageContext.request.contextPath}/common/css/template.css"
			type="text/css" rel="stylesheet">
		...
		<script type="text/javascript">
			...
			{
				var url = "${pageContext.request.contextPath}/sys/bmsrolefunc/beforeDeleteBmsRoleFunc.do";
				if(!$chk(id))
				{
			...
			{
				var url = "${pageContext.request.contextPath}/sys/bmsrolefunc/beforeAddBmsRoleFunc.do";
...
<br />
		<ec:table tableId="BmsRoleFuncExList" items="list" var="m" action=""
			...
			<ec:row>
				<ec:column title="PK" property="id" headerCell="selectAll" alias="ckids">
					...
					<a
						href="${pageContext.request.contextPath}/sys/bmsrolefunc/detailBmsRoleFunc.do?id=${m.id}">
						${m.id} </a>
				</ec:column>
				...
				<ec:column title="操作" property="EEE" sortable="false"
					filterable="false">
					<a
						href="${pageContext.request.contextPath}/sys/bmsrolefunc/beforeEditBmsRoleFunc.do?id=${m.id}">编辑</a>
					<a
