...
public class MenberDAO extends DaoSupport
{
	...
	public List list(LimitInfo limit,String type,String locationId)
	{
		//limit.addFilterProperty(HqlProperty.getEq("status", "1"));
		String where = "";
		if(null != locationId && !locationId.equals("")){
			where =  " and Z.tbTBmsLocationDTO.id = '" + locationId + "'";
		}
		String hql = "from MenberDTO Z "
		...
		{// 非分页, 每页记录小于1时表示不进行分页
			return this.listWithNamePrams(hql + " where Z.type=" + type + where + " " + param[0] + " "
					+ limit.getOrder("Z"), (Map) param[1]);
		} else
		{// 分页查询
			// 查询记录
			List list = this.listWithLimit(hql + " where Z.type=" + type + where + " " + param[0] + " "
			...
			limit.setTotalNum(this.countHql(
					totalHQl + " where Z.type=" + type + where + " " + param[0], (Map) param[1]));
