...
public class ArticleCategoryTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		ArticleCategoryFacade myArticleCategoryFacade = (ArticleCategoryFacade)ac.getBean("myArticleCategoryFacade");
		
		try {
			List<ArticleCategoryDTO> cateList = myArticleCategoryFacade.listByParent("");
//			System.out.println("========================cateList: " + cateList.size());
			for (ArticleCategoryDTO category : cateList) 
			{
				if( value.length()>0){
					ArticleCategoryDTO ca = myArticleCategoryFacade.get(value);
					
					if(id.equals("catCode")){//文章列表页
				...
				if(id.equals("catCode"))//文章列表修改时要显示第二层
				{
					List<ArticleCategoryDTO> category_secondLevel = myArticleCategoryFacade.listByParent(category.getCode());
					// 如果有级联的则将所有级织子项以JS二维数组的形式保存如:["父级编号","值","名称",],[父级编号","值","名称"]
					if (category_secondLevel != null && category_secondLevel.size() > 0) {
						flag1 = true;
						for (ArticleCategoryDTO category2 : category_secondLevel) {
							if( value.length()>0){
								ArticleCategoryDTO ca = myArticleCategoryFacade.get(value);
